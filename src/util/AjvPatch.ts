// patch the node_modules/ajv/lib/dotjs/validate.js

export default function generate_validate(it, $keyword) {
  var out = '';
    ...
    if (arr2) {
      ...
      while (i2 < l2) {
        ...
        if ($shouldUseGroup($rulesGroup)) {
          if ($rulesGroup.type) {
            out += ' if(data1===null) return true;if (' + (it.util.checkDataType($rulesGroup.type, $data)) + ') { ';
          }
  ...
  if ($typeSchema && !$typeChecked && !(it.opts.coerceTypes && $coerceToTypes)) {
    var $schemaPath = it.schemaPath + '.type',
      $errSchemaPath = it.errSchemaPath + '/type',
      $method = $typeIsArray ? 'checkDataTypes' : 'checkDataType';
    out += ' if(data1===null) return true;if (' + (it.util[$method]($typeSchema, $data, true)) + ') {   ';
    var $$outStack = $$outStack || [];
    $$outStack.push(out);
    out = ''; /* istanbul ignore else */
    if (it.createErrors !== false) {
